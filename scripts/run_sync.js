...
const ANASTASIA_PROJECT_ID = "39ace0eb-084a-455e-b058-c6f20cda7f74";
  ...
  for (const chunk of chunks) {
    ...
    try {
      ...
      for (const tx of txList) {
        ...
        // 1. Sync / Update in unified_orders
        try {
          ...
          console.warn(`[Sync] Error updating unified_orders for ${orderRef}:`, crmErr.message);
        }

        // 2. Query local leads table
          ...
          if (isApproved) {
            if (lead.status !== "Оплачено") {
              ...
              if (updateError) {
                ...
                console.log(`Updated existing order ${orderRef} to 'Оплачено' (closed_won)`);
                totalUpdated++;
              }
            }
            ...
            if (lead.status === "Очікує оплати" || lead.status === "Зареєстровано") {
              ...
              if (updateError) {
                ...
                console.log(`Updated failed order ${orderRef} to 'Не оплачено' (declined)`);
                totalUpdated++;
              }
            }
          }
        } else {
          // If transaction is Approved but lead doesn't exist, create it in both tables
          if (isApproved) {
            ...
            try {
              ...
              console.warn("[Sync] Error inserting direct unified order:", uErr.message);
            }
            ...
            if (insertError) {
              ...
              console.log(`Inserted new direct paid order ${orderRef} (closed_won)`);
              totalCreated++;
            }
          }
        }
